...
class TodayTab(ft.Column):
    def __init__(self, c_page):
        ...
        # self.main_page.overlay.append(self.notifications)
        self.spacing = 10
    ...
    def schedule_notification(self, task_title, time_str):
        try:
            # Парсим время ЧЧ.ММ
            ...
            # Планируем
            self.notifications.schedule_notification(
                id=int(datetime.datetime.now().timestamp()), # уникальный ID
                title="Пора за работу! 📌",
                body=f"Задача: {task_title}",
                scheduled_date=target_time
            )
    ...
    def add_task(self):
        ...
        if date and "." in date:
            self.schedule_notification(title, date)
